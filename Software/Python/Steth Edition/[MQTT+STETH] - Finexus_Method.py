...
from    stethoscopeProtocol         import  *   # Status Enquiry
...
scenario_1 = True           # Trigger all regions
scenario_2 = False          # Trigger one region (sound file #1)
scenario_3 = False          # Trigger one region (sound file #2)
scenario_4 = False          # Trigger all regions
# ************************************************************************
...
##            print( "Region #1: Aortic Area" )
            print( "Region #1" )
            sleep( 1.0 )
            blendFileName = 'AORSTE'
            matchIndex = blendByteMatching( blendFileName, blendFiles )
            fileByte = chr( blendInt[matchIndex] )
            startBlending(steth,fileByte)

        elif( crntROI == 2 ):
##            print( "Region #2: Pulmonic Area" )
            print( "Region #2" )
            sleep( 1.0 )
            blendFileName = 'ESMSYN'
            matchIndex = blendByteMatching( blendFileName, blendFiles )
            fileByte = chr( blendInt[matchIndex] )
            startBlending(steth,fileByte)

        elif( crntROI == 3 ):
##            print( "Region #3: Tricuspid Area" )
            print( "Region #3" )
            sleep( 1.0 )
...
##            print( "Region #4: Mitral Area" )
            print( "Region #4" )
            sleep( 1.0 )
            blendFileName = 'RECMIT'
            matchIndex = blendByteMatching( blendFileName, blendFiles )
            fileByte = chr( blendInt[matchIndex] )
            startBlending(steth,fileByte)

        elif( crntROI == 5 ):
            print( "Region #5: Erb's Something" )
            sleep( 1.0 )
            blendFileName = 'RECPUL'
            matchIndex = blendByteMatching( blendFileName, blendFiles )
            fileByte = chr( blendInt[matchIndex] )
            startBlending(steth,fileByte)
...
addr = "00:06:66:D0:E4:37"                                  # MAIN
##addr = "00:06:66:D0:C9:A5"                                  # MAIN
##addr = "00:06:66:D0:C9:BC"                                  # BACKUP
port = 1
