...
void orientRead(int pair) {
  ...
  switch (pair) {
    ...
    case 4:
      //Sensor 5:
      sens[6][0] = ema_filter( double( high.calcMag(high.mx) ), 6, 0);
      sens[6][1] = ema_filter( double( high.calcMag(high.my) ), 6, 1);
      sens[6][2] = ema_filter( double( high.calcMag(high.mz) ), 6, 2);

      //Sensor 6:
      sens[7][0] = ema_filter( double( low.calcMag(low.mx) ), 7, 0);
      sens[7][1] = ema_filter( double( low.calcMag(low.my) ), 7, 1);
      sens[7][2] = ema_filter( double( low.calcMag(low.mz) ), 7, 2);
      break;
