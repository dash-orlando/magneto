...
int 	m = 6, n = NSENS; 				// Number of unknowns, number of equations
...
{	
	...
	// BETA STUFF GOIN' ON HERE!!!
	double r[n][3] = {0};
	double angle[3] = {alpha, beta, gamma};
	double cte = 1e-7*K*(4*3.14159)*(4*3.14159);
	double r_norm = 0;
	for( int i = 0; i < n; i++ )
	{
		// Construct the 'r' term
		r[i][0] = (x - XYZ[i][0]);
		r[i][1] = (y - XYZ[i][1]);
		r[i][2] = (z - XYZ[i][2]);
		r_norm 	= sqrt( r[i][0]*r[i][0] + r[i][1]*r[i][1] + r[i][2]*r[i][2] );
		
		// Get the dot product
		double r_dot_n = r[i][0]*angle[0] + r[i][1]*angle[1] + r[i][2]*angle[2];
		
		sensor_eqn[i] = ( cte*intpow( r_norm, -5 ) * ( 3*z*r_dot_n - angle[2]*r_norm*r_norm ) ) - norm[i]*norm[i];  
	}
}
